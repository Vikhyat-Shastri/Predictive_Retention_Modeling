...
class CustomerSegmentation:
    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numerical_cols = clustering_features.select_dtypes(include=['int64', 'float64']).columns
    ...
    def create_segment_profiles(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for segment in range(self.n_clusters):
            ...
            profile = {
                ...
                'Churn_Rate': segment_data['Churn'].mean() * 100 if 'Churn' in df.columns else None,
                'Avg_Tenure': segment_data['tenure'].mean() if 'tenure' in df.columns else None,
                'Avg_MonthlyCharges': segment_data['MonthlyCharges'].mean() if 'MonthlyCharges' in df.columns else None,
                'Avg_TotalCharges': segment_data['TotalCharges'].mean() if 'TotalCharges' in df.columns else None,
            }
            ...
            for feature in categorical_features:
                if feature in df.columns:
                    ...
                    profile[f'Most_Common_{feature}'] = mode_value[0] if len(mode_value) > 0 else None
    ...
    def visualize_segments(self, df: pd.DataFrame, save_path: str = '../models/segments_visualization.png'):
        ...
        ax2.bar(segment_counts.index, segment_counts.values, color='skyblue', edgecolor='black')
        ...
        if 'Churn' in df.columns:
            ...
            colors = ['red' if x > 30 else 'orange' if x > 20 else 'green' for x in churn_by_segment.values]
            ax3.bar(churn_by_segment.index, churn_by_segment.values, color=colors, edgecolor='black')
            ax3.set_xlabel('Segment')
            ax3.set_ylabel('Churn Rate (%)')
            ax3.set_title('Churn Rate by Segment')
            ax3.set_xticks(range(self.n_clusters))
            ax3.axhline(y=df['Churn'].mean() * 100, color='black', linestyle='--', label='Overall Average')
            ax3.legend()
        ...
        segment_features = df_with_segments.groupby('Segment')[self.feature_names].mean()
        # Normalize for better visualization
        segment_features_norm = (segment_features - segment_features.min()) / (segment_features.max() - segment_features.min())
        sns.heatmap(segment_features_norm.T, annot=False, cmap='YlOrRd', ax=ax6, cbar_kws={'label': 'Normalized Value'})
    ...
    def generate_segment_insights(self, df: pd.DataFrame) -> Dict[str, str]:
        ...
        for segment in range(self.n_clusters):
            ...
            churn_rate = segment_data['Churn'].mean() * 100 if 'Churn' in df.columns else 0
            avg_tenure = segment_data['tenure'].mean() if 'tenure' in df.columns else 0
            avg_monthly = segment_data['MonthlyCharges'].mean() if 'MonthlyCharges' in df.columns else 0
