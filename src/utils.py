...
def tune_hyperparameters(models, X_train, y_train):
    ...
    xg_random = RandomizedSearchCV(models["XGBoost"], xg_params, n_iter=50, cv=5, scoring='f1', n_jobs=-1, random_state=42)
    rfc_random = RandomizedSearchCV(models["Random Forest"], rfc_params, n_iter=30, cv=5, scoring='f1', n_jobs=-1, random_state=42)
