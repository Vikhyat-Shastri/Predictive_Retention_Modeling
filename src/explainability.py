...
class ModelExplainer:
    ...
    def explain_instance(self, X: pd.DataFrame, index: int = 0) -> Dict[str, Any]:
        ...
        prediction_proba = (
            self.model.predict_proba(instance)[0]
            if hasattr(self.model, "predict_proba")
            else None
        )
    ...
    def plot_dependence(
        ...
        shap.dependence_plot(
            feature, shap_values, X, interaction_index=interaction_feature, show=False
        )
    ...
    def get_feature_importance(
        self, X: pd.DataFrame = None, shap_values: np.ndarray = None
    ) -> pd.DataFrame:
        """
    ...
    def create_explanation_report(
        self, X: pd.DataFrame, index: int = 0, save_dir: str = "models/explanations/"
    ) -> Dict[str, Any]:
        """
...
"""
        ...
        for i, (feature, contribution) in enumerate(
            list(explanation["feature_contributions"].items())[:10], 1
        ):
...
if __name__ == "__main__":
    ...
    explainer.plot_summary(
        X_test, save_path="../models/shap_importance.png", plot_type="bar"
    )
