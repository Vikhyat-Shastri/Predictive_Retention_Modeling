...
def plot_numerical_churn(df):
    ...
    Mth = sns.kdeplot(df.TotalCharges[(df["Churn"] == 1)], ax=Mth, color="Blue", shade=True)
    Mth.legend(["No Churn", "Churn"], loc='upper right')
    Mth.set_ylabel('Density')
...
def plot_numerical_features(df):
    ...
    df[numerical_features].hist(bins=15, figsize=(15, 6), layout=(2, len(numerical_features)//2))
    plt.tight_layout()
    plt.show()