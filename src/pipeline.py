...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        binary_cols = [f'{col}_binary' for col in service_cols if f'{col}_binary' in X.columns]
        if binary_cols:
        ...
        if 'ServiceUsageScore' in X.columns and 'MonthlyCharges' in X.columns:
            X['ChargeToServiceRatio'] = X['MonthlyCharges'] / (X['ServiceUsageScore'] + 1)
...
class ChurnPredictionPipeline:
    ...
    def _get_model(self):
        ...
        if self.model_type == 'voting':
            ...
            return StackingClassifier(
                ...
                final_estimator=LGBMClassifier(n_estimators=100, random_state=42, verbose=-1),
                cv=5,
                n_jobs=-1
            )
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, cv_folds: int = 5) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Training {self.model_type} model with {self.resampling_method} resampling")
...
def train_multiple_models(df: pd.DataFrame, test_size: float = 0.2) -> Dict[str, Any]:
    ...
    for name, model_type, resampling in model_configs:
        ...
        try:
            pipeline = ChurnPredictionPipeline(model_type=model_type, resampling_method=resampling)
