...
def create_input_form():
    ...
    st.markdown(
        '<p class="sub-header">📝 Enter Customer Information</p>',
        unsafe_allow_html=True,
    )
    ...
    with col2:
        ...
        total_charges = st.number_input(
            "Total Charges ($)", 0.0, 10000.0, float(tenure * monthly_charges), 50.0
        )
    ...
    if predict_button:
        with st.spinner("Analyzing customer data..."):
            try:
                ...
                # Risk gauge
                st.markdown(
                    '<p class="sub-header">🎚️ Risk Assessment</p>',
                    unsafe_allow_html=True,
                )
                ...
                # Recommendations
                st.markdown(
                    '<p class="sub-header">💡 Recommended Actions</p>',
                    unsafe_allow_html=True,
                )
...
def explanation_page():
    ...
    st.markdown(
        '<p class="main-header">🔍 Prediction Explanation</p>', unsafe_allow_html=True
    )
    ...
    if explain_button:
        with st.spinner("Generating explanation..."):
            try:
                ...
                X_explain = pd.concat(
                    [X.iloc[:1], input_data_encoded], ignore_index=True
                )
                ...
                # Display prediction
                st.markdown(
                    '<p class="sub-header">📊 Prediction Summary</p>',
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    '<p class="sub-header">🎯 Top Feature Contributions</p>',
                    unsafe_allow_html=True,
                )
                ...
                # Interpretation
                st.markdown(
                    '<p class="sub-header">📖 Interpretation Guide</p>',
                    unsafe_allow_html=True,
                )
                ...
                # Detailed breakdown
                st.markdown(
                    '<p class="sub-header">📋 Detailed Feature Analysis</p>',
                    unsafe_allow_html=True,
                )
...
def segmentation_page():
    ...
    st.markdown(
        '<p class="main-header">👥 Customer Segmentation Analysis</p>',
        unsafe_allow_html=True,
    )
    ...
    if segmenter.segment_profiles is not None:
        ...
        # Segment sizes
        st.markdown(
            '<p class="sub-header">📈 Segment Distribution</p>', unsafe_allow_html=True
        )
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    ...
                    textposition="auto",
                    marker=dict(
                        color=px.colors.qualitative.Set3[: len(segment_counts)]
                    ),
                )
            ]
        )
        ...
        # Segment profiles
        st.markdown(
            '<p class="sub-header">📋 Segment Profiles</p>', unsafe_allow_html=True
        )
        ...
        profiles_display["Churn_Rate"] = profiles_display["Churn_Rate"].apply(
            safe_format_pct
        )
        profiles_display["Size_Pct"] = profiles_display["Size_Pct"].apply(
            safe_format_pct
        )
        profiles_display["Avg_MonthlyCharges"] = profiles_display[
            "Avg_MonthlyCharges"
        ].apply(safe_format_currency)
        profiles_display["Avg_TotalCharges"] = profiles_display[
            "Avg_TotalCharges"
        ].apply(safe_format_currency)
        profiles_display["Avg_Tenure"] = profiles_display["Avg_Tenure"].apply(
            safe_format_months
        )
        ...
        st.markdown(
            '<p class="sub-header">⚠️ Churn Risk by Segment</p>', unsafe_allow_html=True
        )
        ...
        # Segment insights
        st.markdown(
            '<p class="sub-header">💡 Segment Insights</p>', unsafe_allow_html=True
        )
...
def model_performance_page():
    ...
    st.markdown(
        '<p class="sub-header">🎯 Model Information</p>', unsafe_allow_html=True
    )
    ...
    st.markdown(
        '<p class="sub-header">📊 Performance Metrics</p>', unsafe_allow_html=True
    )
...
def main():
    ...
    if page == "🏠 Home":
        st.markdown(
            '<p class="main-header">🎯 Customer Churn Prediction System</p>',
            unsafe_allow_html=True,
        )
