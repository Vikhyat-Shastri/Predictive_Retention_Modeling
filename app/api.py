...
async def startup_event():
    ...
    try:
        ...
        models['pipeline'] = ChurnPredictionPipeline.load_model('models/churn_model_pipeline.pkl')
        logger.info("Prediction pipeline loaded successfully")
        
        # Load segmentation model
        try:
            models['segmenter'] = CustomerSegmentation.load_model('models/segmentation_model.pkl')
...
def get_recommendations(prediction: int, probability: float, tenure: int, 
                       monthly_charges: float) -> List[str]:
...
async def predict_batch(customers: List[CustomerData]):
    ...
    try:
        ...
        for i, (customer, pred, proba) in enumerate(zip(customers, predictions, predictions_proba)):
...
async def get_segments_info():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get segment info: {str(e)}")
